...
/* ── HELPER IDENTITY ── */
function getIdentityDetails(devId) {
  var res = { gold: null, green: null, blue: null, raw: devId };
  if (!devId) return res;
  var lv = typeof ROLE_LEVEL !== 'undefined' ? (ROLE_LEVEL[_currentLoginRole] || 1) : 1;
  if (lv < 7) return res;

  // 1. Lấy Tên Xanh dương từ Moon Users (Ingame)
  if (typeof _moonUsers !== 'undefined' && _moonUsers[devId] && _moonUsers[devId].name) {
    res.blue = _moonUsers[devId].name;
  }

  // 2. Lấy Tên Xanh lá / Vàng từ Active Profiles
  if (typeof _identities !== 'undefined') {
    for (var key in _identities) {
      ...
      if (idsArray.indexOf(devId) !== -1) {
        if (profile.merged) {
          res.gold = profile.name || key;
        } else if (!profile.autoGenerated) {
          // Chỉ lấy tên Xanh Lá nếu profile ĐƯỢC FOUNDER TẠO THỦ CÔNG (không chứa cờ autoGenerated)
          res.green = profile.name || key;
        }
      }
    }
  }

  // 3. Fallback lấy xanh dương từ Log nếu bảng user bị thiếu
  if (!res.blue && typeof _allLogs !== 'undefined') {
    for (var i = 0; i < _allLogs.length; i++) {
      if (_allLogs[i].deviceId === devId && _allLogs[i].moonName) {
        res.blue = _allLogs[i].moonName;
        break;
      }
    }
  }
...
function getIdentityName(devId) {
  ...
  var details = getIdentityDetails(devId);
  if (details.gold) return details.gold + '\x00merged';
  if (details.green) return details.green + '\x00founder';
  if (details.blue) return details.blue + '\x00moon';
  return devId;
}
...
function getFilteredLogs() {
  ...
  return _allLogs.filter(function (l) {
    ...
    if (search) {
      ...
      l.screen || '', labelType(l.type), l.authVia ? labelType('login_' + l.authVia) : '', l.moonName || ''].join(' ').toLowerCase();
...
function badgeHtml(type) {
  ...
  if (type === 'login_moon') return '<span class="badge" style="background:rgba(41,128,185,0.1);color:#2980b9;border:1px solid rgba(41,128,185,0.3);"><span class="badge-icon">🌙</span>Ingame</span>';
...
function generateRowHtml(log) {
  ...
  var devIdStr = '';
  if (!log.deviceId) {
    devIdStr = '<span style="color:var(--muted);font-style:italic">Unknown</span>';
  } else {
    var details = getIdentityDetails(log.deviceId);
    var badges = [];
    var isIden = false;

    if (details.gold) {
      badges.push('<span class="dev-badge" style="background:rgba(212,175,55,0.15);color:#b8860b;border-color:rgba(184,134,11,0.4);">⭐ ' + esc(details.gold) + '</span>');
      isIden = true;
    } else {
      if (details.green) {
        badges.push('<span class="dev-badge" style="background:rgba(39,174,96,0.1);color:#27ae60;border-color:rgba(39,174,96,0.3);">👤 ' + esc(details.green) + '</span>');
        isIden = true;
      }
      if (details.blue) {
        badges.push('<span class="dev-badge" style="background:rgba(41,128,185,0.1);color:#2980b9;border-color:rgba(41,128,185,0.3);">🌙 ' + esc(details.blue) + '</span>');
        isIden = true;
      }
    }

    if (!isIden) {
      badges.push('<span class="dev-badge">' + esc(log.deviceId) + '</span>');
    }

    devIdStr = badges.join(' ') + ' <span style="cursor:pointer;font-size:10px;opacity:0.6;vertical-align:middle;margin-left:4px;" onclick="openLinkIdentity(\'' + esc(String(log.deviceId)) + '\')" title="Link to Profile">🔗</span>';
  }
  ...
  try {
    ...
    filteredCols.forEach(function (col) {
      ...
      if (top.length === 0) { html += '<div class="ip-empty">No data</div>'; }
        ...
        var details = entry.devIds.length > 0 ? getIdentityDetails(entry.devIds[0]) : null;
        var profileTitle = '';
        if (details && (details.gold || details.green || details.blue)) {
          profileTitle += '<div style="font-weight:900;font-size:14px;margin-bottom:4px;display:flex;align-items:center;gap:6px;flex-wrap:wrap;">';
          if (details.gold) {
            profileTitle += '<span style="color:#b8860b;">⭐ ' + esc(details.gold) + '</span>';
          } else {
            if (details.green) profileTitle += '<span style="color:#27ae60;">👤 ' + esc(details.green) + '</span>';
            if (details.blue) profileTitle += '<span style="color:#2980b9;">🌙 ' + esc(details.blue) + '</span>';
          }
          profileTitle += ' <span style="cursor:pointer;font-size:12px;opacity:0.6;" onclick="openLinkIdentity(\'' + esc(String(entry.devIds[0] || '')) + '\')" title="Link to Profile">🔗</span></div>';
        }
